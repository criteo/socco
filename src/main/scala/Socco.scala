...
class Socco(val global: Global) extends Plugin {
  ...
  implicit class RegexContext(sc: StringContext) {
    def regex = new scala.util.matching.Regex(sc.parts.mkString, sc.parts.tail.map(_ => "_"): _*)
  }

  override def processOptions(options: List[String], callback: (String) => Unit) = {
    ...
    options.foreach {
      case regex"out:(.*)$directory" =>
        Try(out = new java.io.File(directory)).fold(error, identity)
      ...
      case regex"package_([^:]*)$packageName:(.*)$url" =>
        Try(packages = packages + (packageName -> url)).fold(error, identity)
      case oops =>
        error(new Exception(s"Option not recognized, $oops"))
    }
  }
  ...
  private object Component extends PluginComponent {
    ...
    class SoccoPhase(prev: Phase) extends StdPhase(prev) {
      ...
      def splitInBlocks(unit: CompilationUnit): (Option[String], Seq[(Block,Block)]) = {
        ...
        def isComment(line: String): Option[String] = """^\s*//\s*(.*)$""".r.unapplySeq(line).flatMap(_.lift(0))
        val (_, blocks) = maybeTitle.fold(lines)(_ => lines.tail).foldLeft(((maybeTitle.fold(0)(_ => lines.head.length + 1)) -> List.empty[Either[Block,Block]])) {
          case ((offset, blocks), line) =>
            (offset + line.length + 1) -> (blocks match {
              ...
              case block :: rest =>
                isComment(line).fold[List[Either[Block,Block]]]
                  { block match {
                      case Left(_) => Right(Block(offset, line)) :: block :: rest
                      case Right(block) => Right(block.copy(text = s"${block.text}\n${line}")) :: rest
                    }
                  }
                  { comment =>  block match {
                      case Left(block) => Left(block.copy(text = s"${block.text}\n${comment}")) :: rest
                      case Right(_) => Left(Block(offset, comment)) :: block :: rest
                    }
                  }
            })
        }
      ...
      def tokenize(unit: CompilationUnit): Seq[Token] = {
        ...
        if(unit.isJava) sys.error("😂") else {
          (new syntaxAnalyzer.UnitParser(unit) { override def newScanner = new Tokenizer }).parse()
        }

        tokens.toSeq.sortBy(_._1).map { case (start, (length, code)) =>
      ...
      def typeTokens(unit: CompilationUnit, tokens: Seq[Token]): Seq[Token] = {
        ...
        def scalaDocLink(symbol: Symbol): Option[String] = {
          def go(symbol: Symbol): Option[(String,String,String)] = symbol match {
            case NoSymbol =>
              None
            case s if s.isRoot =>
              None
            case p: ModuleSymbol if p.hasPackageFlag =>
              Some((go(p.owner).fold("") { case (p, s, _) => p + s } + p.name, "/", "/index.html"))
            case p: PackageClassSymbol =>
              Some((go(p.owner).fold("") { case (p, s, _) => p + s } + p.name, "/", "/index.html"))
            case o: ModuleSymbol =>
              Some((go(o.owner).fold("") { case (p, s, _) => p + s } + o.name + "$", "$", ".html"))
        ...
        def maybeTypeSymbol(symbol: Symbol): Option[TypeAnnotation] = {
          def findPackage(symbolName: String) = packages.find {
            case (name, _) => (name == symbolName) || (symbolName.startsWith(name + "."))
          }
              ...
              findPackage(m.owner.fullName).flatMap { case (_, link) =>
                scalaDocLink(m).map { encodedMethodName =>
                  ...
                  TypeAnnotation(
                    ...
                    s"${m.fullName}${m.info.toString.replaceAll(s"(=>\\s*)?\\Q$returnType\\E${'$'}", s": $returnType")}"
                  )
                }
              }
            case t: TermSymbol =>
      ...
      def generate(unit: CompilationUnit, tokens: Seq[Token], title: String, blocks: Seq[(Block,Block)]): Unit = {
        ...
        def formatSourceCode(block: Block, tokens: Seq[Token]): String = {
          ...
          def style(token: Token) = token.style match {
            case Keyword =>
            ...
            case Identifier(Some(t)) =>
              s"""<span class="identifier"><a target="doc" href="${t.link}" data-tooltip="<span>${t.display}</span>">${escape(token)}</a></span>"""
            case _ => escape(token)
          }

          val (lastPosition, html) = remapTokens(tokens, block.start, block.end).
            foldLeft(0 -> "") { case ((i, text), token) =>
              token.end -> (
                if(token.start > i) {
        ...
        def formatComments(block: Block, packages: Map[String,String]): String = {
          import laika.api._
          import laika.render._
          import laika.parse.markdown._
          import laika.tree.Elements.{ ExternalLink }

          def findPackage(url: String) = packages.find {
            case (name, _) => url.startsWith(name.replaceAll("[.]", "/"))
          }.map(_._2)
          Transform.from(Markdown).to(HTML).rendering { out =>  {
            case ExternalLink(content, url, _, _) if findPackage(url).isDefined =>
              out << s"""<a target="doc" href="${findPackage(url).get}/${url}">""" << content << """</a>"""
          } }.fromString(block.text).toString
        }
        ...
        val html = s"""
          ...
          ${blocks.map { case (comment, code) =>
            s"""
              <section class="${if(comment.text.trim.matches("@\\w+")) comment.text.trim.drop(1) else ""}">
                <div class="comment ${if(comment.text.trim.isEmpty) "empty" else ""}">${formatComments(comment, packages)}</p></div>
                <div class="code ${if(code.text.trim.isEmpty) "empty" else ""}">${formatSourceCode(code, tokens)}</div>
              </section>
            """
          }.mkString}
          <section id="footer">
  ...
  def fromClasspath(resource: String): String = {
    Option(getClass.getResource(resource)).map { res =>
